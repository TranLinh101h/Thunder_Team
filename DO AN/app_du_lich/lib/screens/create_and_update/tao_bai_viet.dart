...
class TaoBaiVietState extends State<TaoBaiViet> {
  final _picker =
      ImagePicker(); //khai báo để dùng các thuộc tính liên quan đến ảnh
  File? _imageFile;
  ...
  Min_Dia_Danh
      selectDiaDanh; // Man create 14/01/2022 | Biến chứa giá trị ta gửi qua
  ...
  void _taoBaiViet() async {
    ...
    ApiResponse response = await taoBaiViet(_txtContent.text,
        selectDiaDanh.id.toString()); // Giong nhu tra ve response.body

    if (response.error == null) {
      // Neu tạo bài thành công
      if (image != null) // có ảnh
      {
        {
          ApiResponse responseID =
              await getIdBaiViet(); // lấy id bài viết để thêm ảnh cho bài vừa tạo
          Map<dynamic, dynamic> idBaiViet =
              responseID.data as Map<dynamic, dynamic>;
          int i = idBaiViet['id'];
  ...
  Widget _createButton(IconData icon, String txt) {
    // er - Tạo button ở dưới cùng
    return Container(
                  ...
                  onPressed: () {
                    setState(() {
                      if (_formKey.currentState!.validate()) {
                        //1. Kiem tra xem nguoi ta co nhap gi khong
                        _loading = !_loading;
                        _taoBaiViet();
                      }
                    });
                            ...
                            child: ListTile(
                                ...
                                subtitle: Row(
                                  children: [
                                    ...
                                    Text(
                                      selectDiaDanh.ten_dia_danh != 'home'
                                          ? selectDiaDanh.ten_dia_danh
                                          : "",
                                      style: const TextStyle(
