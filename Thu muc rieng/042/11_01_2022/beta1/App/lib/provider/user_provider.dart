...
Future<ApiResponse> updateUser(String name, String gioi_Tinh, String email, String sdt, String? img) async {
  ApiResponse apiResponse = ApiResponse();
  try{
    String token = await getToken();
...
Future<ApiResponse> updatePassUser(String password) async {
  ...
  try{
    String token = await getToken();
    final response = await http.put(Uri.parse(updatePassURL),
      headers: {'Accept': 'application/json', 'Authorization': 'Bearer $token'},
     body: {
      'password': password,
    });
